 ...
 * Расчет индикаторов.
 */
import {SMA, EMA, RSI, Stochastic} from '@debut/indicators';
...
/* TODO: change this*/
export function stochastic(high: number, low: number, close: number, length: number) {
  const stoch = new Stochastic(length);
  stoch.nextValue(high, low, close);
}
